...
let URL = "https://teachablemachine.withgoogle.com/models/IwWookARf/";
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                ...
                break;
            case '2':
                if (time >= 8.0 && time <= 10.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if ((time >= 14.0 && time <= 16.0 && !poseState.firstWindowTriggered) ||
                    (time >= 15.2 && time <= 17.2 && !poseState.secondWindowTriggered)) {
                    if (time <= 14.0) {
                ...
                break;
            case '4':
                if (time >= 23.4 && time <= 25.2 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 26 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/IwWookARf/");
    }
}
